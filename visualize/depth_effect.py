...
def depth_effect(filename_s, filename_l):
    ...
    depth = range(1, 7)
    dict = {0: "mAP", 1: "Rank-1"}
    fig, (ax1, ax2) = plt.subplots(2, 2, figsize=(12, 12), dpi=400)
    ...
    for i in range(len(labels)):
        ...
        if i < 3:
            ...
            style = '-'
            linewidth = 3
        ax1[0].plot(depth, map_data[i], label=labels[i], color=colors[i], linestyle=style, linewidth=linewidth)
        ax1[0].set_ylabel("mAP (%)", fontsize=20)

        for x, y in zip(depth, map_data[-1]):
            ax1[0].text(x - 0.25, y - 0.20, '%.1f' % y, fontdict={'fontsize': 13})
        # ax1[0].set_title("Market1501  mAP", fontsize=18)

        ax1[1].plot(depth, r1_data[i], label=labels[i], color=colors[i], linestyle=style, linewidth=linewidth)
        ax1[1].set_ylabel("Rank-1 (%)", fontsize=20)

        for x, y in zip(depth, r1_data[-1]):
            ax1[1].text(x - 0.25, y - 0.04, '%.1f' % y, fontdict={'fontsize': 13})
    ...
    for i in range(len(labels)):
        ...
        if i < 3:
            ...
            style = '-'
            linewidth = 3
        ax2[0].plot(depth, map_data[i], label=labels[i], color=colors[i], linestyle=style, linewidth=linewidth)
        ax2[0].set_ylabel("mAP (%)", fontsize=20)

        for x, y in zip(depth, map_data[-1]):
            ax2[0].text(x - 0.25, y - 0.27, '%.1f' % y, fontdict={'fontsize': 13})
        # ax2[0].set_title("MSMT17  mAP", fontsize=18)
        ax2[1].plot(depth, r1_data[i], label=labels[i], color=colors[i], linestyle=style, linewidth=linewidth)
        ax2[1].set_ylabel("Rank-1 (%)", fontsize=20)

        for x, y in zip(depth, r1_data[-1]):
            ax2[1].text(x - 0.25, y - 0.05, '%.1f' % y, fontdict={'fontsize': 13})
    ...
    fig.legend(handles, labels, loc='upper center', fontsize=17, bbox_to_anchor=(0.52, 1.0),
               bbox_transform=fig.transFigure, ncol=7)
    fig.tight_layout(pad=5.5)
    plt.savefig('depth_effect.pdf', bbox_inches='tight')
    plt.show()
