...
class Stem(nn.Module):
    def __init__(self, in_dim=3, out_dim=96, patch_size=2,stride_size=2):
        super().__init__()
        self.patch_size = to_2tuple(patch_size)
        self.stride_size = to_2tuple(stride_size)
        self.proj = nn.Conv2d(in_dim, out_dim, kernel_size=patch_size, stride=stride_size)
    ...
    def forward(self, x):
        ...
        x = self.norm(x)
        H, W = H // self.stride_size[0], W // self.stride_size[1]
        return x, (H, W)
...
class PatchEmbed(nn.Module):
    def __init__(self, in_ch=3, out_ch=96, patch_size=2,kernel_size=3,stride=2,padding=1, with_pos=True):
        super().__init__()
        self.patch_size = to_2tuple(patch_size)
        self.stride_size = to_2tuple(stride)
        self.proj = nn.Conv2d(in_ch, out_ch, kernel_size=kernel_size, stride=stride, padding=padding)
...
class ResTV2(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, embed_dims=[96, 192, 384, 768],
                 num_heads=[1, 2, 4, 8], drop_path_rate=0., camera=0, view_num=0,
    ...
    def forward(self, x, cam_label=None, view_label=None):
        ...
        x = self.norm(x)
        x = x.permute(0, 2, 1).reshape(B, -1, H, W)
        cls = self.avg_pool(x).transpose(1, 2).squeeze(-1)
    ...
    def load_param(self, model_path):
        ...
        for i in param_dict['model']:
            ...
            if j in self.state_dict().keys():
                self.state_dict()[j].copy_(param_dict['model'][i])


@register_model
def restv2_tiny(img_size, sie_xishu, stride_size, drop_rate, drop_path_rate, attn_drop_rate, num_classes, camera,
                 view,pretrained=False,
                **kwargs):  # 82.3|4.7G|24M -> |3.92G|30.37M   4.5G|30.33M
    model = ResTV2(num_classes=num_classes, embed_dims=[96, 192, 384, 768], depths=[1, 2, 6, 2],camera=camera, view_num=view, **kwargs)
    return model


@register_model
def restv2_small(img_size, sie_xishu, stride_size, drop_rate, drop_path_rate, attn_drop_rate, num_classes, camera,
                 view, pretrained=False,
                 **kwargs):  # 83.6|7.0G|35M   -> |5.78G|40.94M
    model = ResTV2(num_classes=num_classes, embed_dims=[96, 192, 384, 768], depths=[1, 2, 12, 2],camera=camera, view_num=view, **kwargs)
    return model


@register_model
def restv2_base(img_size, sie_xishu, stride_size, drop_rate, drop_path_rate, attn_drop_rate, num_classes, camera,
                 view, pretrained=False, **kwargs):  # 84.4|10.2G|52M -> |7.25G|55.75M
    model = ResTV2(num_classes=num_classes, embed_dims=[96, 192, 384, 768], depths=[1, 3, 16, 3],camera=camera, view_num=view, **kwargs)
    return model


@register_model
def restv2_large(img_size, sie_xishu, stride_size, drop_rate, drop_path_rate, attn_drop_rate, num_classes, camera,
                 view, pretrained=False, **kwargs):  # 85.3|39.6|218M -> |14.09G|98.61M
    model = ResTV2(num_classes=num_classes, num_heads=[2, 4, 8, 16], embed_dims=[128, 256, 512, 1024],
                   depths=[2, 3, 16, 2], camera=camera, view_num=view, **kwargs)
    return model

# model = model = ResTV2(num_classes=751, num_heads=[2, 4, 8, 16], embed_dims=[128, 256, 512, 1024],
#                    depths=[2, 3, 16, 2], camera=6, view_num=0)
# model.load_param('/15127306268/wyh/UIS/pth/restv2_large_224.pth')
