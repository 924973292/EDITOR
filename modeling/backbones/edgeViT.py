...
class EdgeVit(nn.Module):
    ...
    def __init__(self, depth=[1, 2, 5, 3], img_size=256, in_chans=3, num_classes=1000,
    ...
    def forward(self, x, cam_label=None, view_label=None):
        ...
        x = torch.cat([cls, x.permute(0, 2, 1)], dim=-2)
        return x
...
@register_model
def edgevit_s(img_size, sie_xishu, stride_size, drop_rate, drop_path_rate, attn_drop_rate, num_classes, camera,
              view, pretrained=True, **kwargs):
    model = EdgeVit(num_classes=num_classes,
                    depth=[1, 2, 5, 3],
                    embed_dim=[48, 96, 240, 384], head_dim=48, mlp_ratio=[4] * 4, qkv_bias=True,
                    norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4, 2, 2, 1], **kwargs)
    model.default_cfg = _cfg()
