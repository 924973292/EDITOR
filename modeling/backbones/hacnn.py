...
class HACNN(nn.Module):
    ...
    def forward(self, x):
        ...
        # ============== Feature generation ==============
        # global branch
        x_global_ = F.avg_pool2d(x3_out,
                                 x3_out.size()[2:]
                                 ).view(x3_out.size(0), x3_out.size(1))
        x_global_ = self.fc_global(x_global_)
        x_global = self.classifier_global(x_global_)
        b, c = x_global_.shape
        x_global_ = x_global_.reshape(b, c, 1, 1)
        # local branch
        if self.learn_region:
            x_local_list = []
            for region_idx in range(4):
            ...
            x_local_ = torch.cat(x_local_list, 1)
            x_local_ = self.fc_local(x_local_)
            x_local = self.classifier_local(x_local_)
            b, c = x_local_.shape
            x_local_ = x_local_.reshape(b, c, 1, 1)
        if not self.training:
            # l2 normalization before concatenation
            if self.learn_region:
                return torch.cat([x_global_, x_local_], dim=1).reshape(b, 2*c)

        if self.learn_region:
            return x_global, x_global_, x_local, x_local_
