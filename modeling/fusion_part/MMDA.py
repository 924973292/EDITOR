...
class ChannelEnchance(nn.Module):
    def __init__(self, dim, outdim):
        super().__init__()
        self.local_1 = ChannelUnite(dim=dim, outdim=outdim, kernel=1)
        self.local_3 = ChannelUnite(dim=dim, outdim=outdim, kernel=3)
        self.local_5 = ChannelUnite(dim=dim, outdim=outdim, kernel=5)
        self.local_7 = ChannelUnite(dim=dim, outdim=outdim, kernel=7)
        self.act = nn.Sigmoid()
    ...
    def forward(self, x, weight, height):
        ...
        c = self.act(c_1 + c_3 + c_5 + c_7).reshape(b, c, 1, 1)
        out = (x * c).flatten(2).transpose(1, 2)
        return out
...
class FuseUnit(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.mlp = ChannelEnchance(dim=dim, outdim=dim)
