...
class Part_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 判断max_index_set的非0元素个数
        # count = torch.count_nonzero(max_index_set)//B
        # print(f'Count:{count}')
...
class Person_Token_Select(nn.Module):
    ...
    def forward_single(self, feat, attn):
        # part_num, part_inx = self.part_select(attn)
        # part_inx = part_inx + 1
        # parts = []
        # B, num = part_inx.shape
        # for i in range(B):
        #     parts.append(feat[i, part_inx[i, :]])
...
class Person_Token_SelectC(nn.Module):
    def __init__(self, dim, ratio=0.5):
        super(Person_Token_SelectC, self).__init__()
        self.ratio = ratio
    ...
    def forward(self, features, img_path, mode=1):
        ...
        if self.training:
            ...
            pre_fix = '/13559197192/wyh/UNIReID/data/RegDB/test/'
